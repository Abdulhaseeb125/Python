...
class SellCanvas:
    ...
    def frame_1(self):
        ...
        self.go_back = ctk.CTkButton(self.purchase_frame, text="<<<", width=20, fg_color='transparent',
                                     command=self.go_back)
        self.go_back.place(relx=0.1, rely=0.1, anchor=ctk.CENTER)

        self.section = ctk.CTkLabel(self.purchase_frame, text="Selling Details", font=("arial", 20))
        ...
        self.imei2_entry.configure(validatecommand=(self.imei2_entry.register(lambda P: P.isdigit() or P == ""), '%P'))
        self.imei2_entry.place(relx=0.5, rely=0.5, anchor=ctk.CENTER)


        self.customer_entry = ctk.CTkEntry(self.purchase_frame, width=180, placeholder_text="Customer Name")
        self.customer_entry.place(relx=0.5, rely=0.6, anchor=ctk.CENTER)
        ...
        self.date_entry.insert(1,self.date)
        self.date_entry.place(relx=0.4, rely=0.7, anchor=ctk.CENTER)

        self.time = time.strftime("%H:%M:%S")
        self.time_entry = ctk.CTkEntry(self.purchase_frame, width=100,placeholder_text=self.date)
        self.time_entry.insert(1,self.time)
        self.time_entry.place(relx=0.61, rely=0.7, anchor=ctk.CENTER)

        self.submit = ctk.CTkButton(self.purchase_frame,command=self.submit, text="Add", fg_color="black",corner_radius=30,border_width=1)
    ...
    def submit(self):
        ...
        self.customer_name = self.customer_entry.get()
        self.sdate = self.date_entry.get()
        self.stime = self.time_entry.get()
        # insert data into the table
        c.execute('''INSERT INTO sold_data(model_name,brand_name, imei1, imei2,customer_name, date, time) 
                     VALUES (?, ?, ?, ?, ?, ?, ?)''', (self.model_name,self.brand_name, self.imei1_number, self.imei2_number, self.customer_name,self.sdate, self.stime))
        conn.commit()
        if conn:
